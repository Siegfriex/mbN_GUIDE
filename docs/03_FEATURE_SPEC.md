...
| G-06 Place Detail | CONTEXT/TRUST/INTENT; understand place relevance. | `/place/:placeId`; preview/deep link. | resolved Place, context relations, locale → detail state. | `place_detail_viewed` {placeId, entryPoint}. | Identity/provenance survive missing media; relation failure is section empty; unavailable target is S-07. FRONT consumes; PY supplies approved relations. |
...
| D-02 Story Bundle | CONTEXT; understand a cultural theme. | `/story/:storyId`; story card/deep link. | Story, Article relations, locale → story detail. | `story_opened` {storyId, source}. | Unknown ID is S-07; story can show relation-empty state without partner CTA invention. FRONT/PY relation contract. |
...
| L-03 Live Detail | CONTEXT/ACTION; inspect a session. | `/live/:sessionId`; card/deep link. | resolved LiveSession → detail state. | `live_session_opened` {sessionId, status}. | Live/upcoming/replay/unavailable labels persist; invalid ID is S-07. FRONT/PY session projection. |
| L-04 Live Status | TRUST; know what can actually be watched. | Live home/detail; status presentation. | `status`, scheduled/replay availability → visible status. | `live_status_viewed` {sessionId?, status}. | Never present replay/upcoming as live or invent viewers. FRONT consumes status. |
